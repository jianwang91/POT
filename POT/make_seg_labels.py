...
def _work(process_id, model, dataset, args):
    ...
    with torch.no_grad(), cuda.device(process_id):
        ...
        for iter, pack in enumerate(data_loader):
            ...
            cam_dict['IS_CAM'] = cam_dict['IS_CAM1']
            # cam_dict['IS_CAM'] = cam_dict['IS_CAM'] + cam_dict['IS_CAM1']
            keys = cam_dict['keys']
            cams = F.interpolate(torch.tensor(cam_dict['IS_CAM']).unsqueeze(1), edge.shape[1:], mode='bilinear',
...
if __name__ == '__main__':
    ...
    args.sem_seg_out_dir = os.path.join(args.session_name, 'pseudo_label_fig4')
