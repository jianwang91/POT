...
def validate(model, data_loader):
    val_dataset = data_voc.VOC12SegmentationDataset2('data/train_voc.txt',
                                                     label_dir='path/to/VOCdevkit/VOC2012/SegmentationClassAug',
                                                     voc12_root='path/to/VOCdevkit/VOC2012',
                                                     crop_size=None,
                                                     crop_method="none",
                                                     )
    ...
    for i, pack in enumerate(val_data_loader):
        ...
        for i_clip in range(n):
            cam_dict = np.load(
                os.path.join("path/to/CLIP_ES_refined_CAM/cams_71",
...
def train():
    ...
    parser.add_argument("--dataset_root", default="path/to/VOCdevkit/VOC2012", type=str)
    parser.add_argument("--seed", default=15, type=int)
    parser.add_argument('--img_root', type=str,
                        default='path/to/VOCdevkit/VOC2012/JPEGImages')
    ...
    if args.dataset == 'voc':
        dataset_root = 'path/to/VOCdevkit/VOC2012'
    ...
    for ep in range(args.max_epoches):
        ...
        for step, pack in enumerate(train_data_loader):
            ...
            with ((autocast())):
                ...
                norm_clip = norm_clip * valid_mask_lowres[:, 1:]

                N_L, _, H_L, W_L = img.shape
                outputs = model.forward(img, valid_mask_lowres, False, None, norm_clip, pack['name'])
                score = outputs['score']
                cam_class = outputs['cam_class'] * valid_mask_lowres
                cam_add = outputs['cam_add'] * valid_mask_lowres
                cam_class = cam_class / (F.adaptive_max_pool2d(cam_class, (1, 1)) + 1e-5)
                cam_add = cam_add / (F.adaptive_max_pool2d(cam_add, (1, 1)) + 1e-5)

                lossCon = torch.mean(torch.abs(cam_class - cam_add)) * 10
                lossCLS = F.multilabel_soft_margin_loss(score, label)
                losses = 0.5 * lossCLS + lossCon

                avg_meter.add({'lossCLS': lossCLS.item(),
                               'lossCon': lossCon.item()})
            ...
            if (optimizer.global_step - 1) % args.print_freq == 0:
                ...
                print('step:%5d/%5d' % (optimizer.global_step - 1, max_step),
                      ...
                      'lossAdd:%.4f' % (avg_meter.pop('lossCon')),
            ...
            if (optimizer.global_step - 1) % args.tf_freq == 0:
                ...
                cam_class = F.interpolate(cam_class, img_8.shape[1:], mode='bilinear')[0].detach().cpu().numpy()

                CAM = visualization.generate_vis(cam_class, None, img_8,
                                                 func_label2color=visualization.VOClabel2colormap, threshold=None,
                                                 norm=False)
                cam_add = F.interpolate(cam_add, img_8.shape[1:], mode='bilinear')[0].detach().cpu().numpy()
                cam_add = visualization.generate_vis(cam_add, None, img_8,
                                                     func_label2color=visualization.VOClabel2colormap, threshold=None,
                                                     norm=False)

                # tf record
                tblogger.add_images('CAM', CAM, optimizer.global_step)
                tblogger.add_images('cam_add', cam_add, optimizer.global_step)

            if (optimizer.global_step - 1) % args.val_freq == 0 and optimizer.global_step > 10:
                miou = validate(model, val_data_loader)
                if optimizer.global_step > 2500:
